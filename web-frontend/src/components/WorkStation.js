...
const WorkStation = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/work-station`);
        setRecords(response.data);
        localStorage.setItem('workStationRecords', JSON.stringify(response.data));
      } catch (error) {
        const savedRecords = localStorage.getItem('workStationRecords');
        if (savedRecords) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/api/work-station`, form);
      const updatedData = await axios.get(`${API_BASE_URL}/api/work-station`);
      setRecords(updatedData.data);
    } catch (error) {
      const newRecords = [...records, form];
      setRecords(newRecords);
      localStorage.setItem('workStationRecords', JSON.stringify(newRecords));
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await axios.put(`${API_BASE_URL}/api/work-station/${editingId}`, form);
      const updatedData = await axios.get(`${API_BASE_URL}/api/work-station`);
      setRecords(updatedData.data);
    } catch (error) {
  ...
  const handleDelete = async (id, index) => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/api/work-station/${id}`);
      const updatedData = await axios.get(`${API_BASE_URL}/api/work-station`);
      setRecords(updatedData.data);
    } catch (error) {
